...
const PersonasComponent = () => {
  ...
  const showNotification = (message, type = 'info') => {
    ...
    setTimeout(() => setNotification({ message: '', type: '' }), type === 'alert' ? 12000 : 5000); 
  };
    ...
    const init = async () => {
      if (window.ethereum) {
        try {
          ...
          if (!deployedNetwork) {
            showNotification("Contrato no desplegado en esta red. Revisa tu configuración.", 'error');
            return;
          }
          ...
          showNotification("Conexión con la red y contrato exitosa.", 'success');
        } catch (error) {
          console.error("Error al inicializar:", error);
          showNotification("Error al conectar con MetaMask o cargar el contrato.", 'error');
        }
      } else {
        showNotification("MetaMask no detectado. Instala la extensión para usar la aplicación.", 'error');
      }
    };
    init();
  }, []);
  ...
  const verificarIdYPrecargar = async () => {
    ...
    try {
        const result = await contract.methods.obtenerPersonaPorId(idPersonaCompleto).call();
        const idStr = String(result.id || 0);

        if(result && result.nombres && result.nombres !== "" && idStr === idPersonaCompleto) {
            ...
            showNotification(`ID ${idPersonaCompleto} encontrado. Campos precargados para actualización.`, 'success');
        } else {
            // ID NO EXISTE o está vacío. Permitir registro inicial completo.
            resetCamposCompletos(); 
            setIsIDVerified(true);
            showNotification(`ID ${idPersonaCompleto} NO encontrado. Puedes registrar una persona completa con este ID.`, 'info');
        }
  ...
  const registrarPersonaEsencial = async () => {
    ...
    if (!cedulaEsencial || !nombresEsencial || !apellidosEsencial) {
        showNotification("Los campos Cédula, Nombres y Apellidos son obligatorios para el registro esencial.", 'error');
        return;
    }
    try {
      await window.ethereum.request({ method: "eth_requestAccounts" });
      
      // Enviamos la transacción y esperamos el recibo (receipt)
      const receipt = await contract.methods
        .registrarPersonaEsencial(cedulaEsencial, nombresEsencial, apellidosEsencial, account)
        .send({ from: account });
      
      let newId = null;

      // 1. Capturar el evento 'PersonaRegistrada'
      if (receipt.events && receipt.events.PersonaRegistrada) {
        // Acceder al ID del evento
        newId = receipt.events.PersonaRegistrada.returnValues.id;
      } 
      // Nota: Si el evento no fuera indexed, Web3 requiere un método diferente (getPastEvents), pero para eventos simples, receipt.events es lo más directo.
      ...
      if (newId) {
          ...
          // **OPTIMIZACIÓN:** Precargar la ID en el formulario completo y marcar como verificado.
          setIdPersonaCompleto(newId);
          setIsIDVerified(true);
          setCedulaCompleto(cedulaEsencial); // Precarga los datos esenciales también
          setNombresCompleto(nombresEsencial);
          setApellidosCompleto(apellidosEsencial);
          
      } else {
          showNotification("Persona esencial registrada con éxito, pero la ID no pudo ser recuperada automáticamente. Por favor, use la función de consulta por ID para encontrarla.", 'success');
      }
      ...
      console.error(err);
      showNotification("Error al registrar persona esencial. (Verifica si ya existe o si la transacción fue rechazada)", 'error');
    }
  };
  ...
  const registrarPersonaCompleta = async () => {
    ...
    if (!idPersonaCompleto || !cedulaCompleto || !nombresCompleto || !apellidosCompleto) {
        showNotification("Los campos ID Persona, Cédula, Nombres y Apellidos del Registro Completo son obligatorios.", 'error');
        return;
    }
    try {
      ...
      console.error(err);
      showNotification("Error al registrar persona completa. (Verifica el ID, permisos o si la transacción fue rechazada)", 'error');
    }
  };
  ...
  const consultarPersonaPorCI = async () => {
    ...
    try {
      const result = await contract.methods.obtenerPersonaPorCI(cedulaConsulta).call();
      if(result && result.nombres && result.nombres !== "") { 
          setPersona(result);
          showNotification(`Persona con CI ${cedulaConsulta} encontrada.`, 'info');
      } else {
          setPersona(null);
          showNotification(`No se encontró persona con CI ${cedulaConsulta}.`, 'info');
      }
    } catch (err) {
      console.error(err);
      setPersona(null);
      showNotification("Error al consultar persona por CI. Asegúrate que la cédula sea válida.", 'error');
    }
  };
  ...
  const consultarPersonaPorId = async () => {
    ...
    try {
      const result = await contract.methods.obtenerPersonaPorId(idConsulta).call();
      const idStr = String(result.id || 0); 
      ...
      if(result && result.nombres && result.nombres !== "" && idStr !== "0") {
          ...
          setPersonaPorId(null);
          showNotification(`No se encontró persona con ID ${idConsulta}.`, 'info');
      }
    } catch (err) {
      console.error(err);
      setPersonaPorId(null);
      showNotification("Error al consultar persona por ID. Asegúrate que el ID sea válido.", 'error');
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="lg:w-2/3 space-y-8">
                ...
                <h2 className={sectionTitleClasses}>1. Registro Esencial (Nuevas Personas)</h2>
                <p className="text-sm text-gray-600 mb-4">Solo registra la información mínima y la wallet. La ID generada será mostrada y precargada en la sección de Actualización.</p>
                ...
                <div className={containerClasses}>
                <h2 className={sectionTitleClasses}>2. Registro Completo / Actualización</h2>
                <p className="text-sm text-gray-600 mb-4">Completa los datos de una persona existente o registra una nueva con toda la información.</p>
                ...
                <div className="flex space-x-2 mb-4 items-center">
                    <input 
                        ...
                        onChange={(e) => {setIdPersonaCompleto(e.target.value); setIsIDVerified(false);}} 
                    ...
                    {isIDVerified && <span className="p-2 border rounded-lg bg-green-100 text-green-700 font-semibold flex items-center">ID Verificado ✅</span>}
                </div>
                ...
                <fieldset disabled={!isIDVerified} className="space-y-4">
                    <legend className="text-sm font-semibold text-gray-700 border-t pt-2">Datos a Registrar/Actualizar:</legend>
                ...
                <div className="flex justify-start space-x-4 mt-4 pt-4 border-t">
                    ...
                    <button 
                    onClick={() => {setIdPersonaCompleto(''); resetCamposCompletos();}}
            ...
            <div className="lg:w-1/3 space-y-8">
                ...
                <div className={containerClasses}> 
                    <h2 className={sectionTitleClasses}>3. Consulta por Cédula (CI)</h2>
                    ...
                    {persona && (
                        <div className={resultCardClasses + " mt-4"}>
                            <h4 className="text-lg font-bold mb-2 text-blue-800">✅ Resultado por Cédula</h4>
                            ...
                            <p><strong>Wallet:</strong> <span className="font-mono text-sm break-all">{persona.wallet}</span></p>
                        </div>
                    )}
                </div>
                ...
                <div className={containerClasses}>
                    ...
                    {personaPorId && (
                        <div className={resultCardClasses + " border-indigo-700 bg-indigo-50 mt-4"}>
                            <h4 className="text-lg font-bold mb-2 text-indigo-800">🔎 Resultado por ID</h4>
                            <p><strong>ID:</strong> {personaPorId.id}</p> 
                            <p><strong>Nombres:</strong> {personaPorId.nombres}</p>
                            <p><strong>Apellidos:</strong> {personaPorId.apellidos}</p>
                            <p><strong>Cédula:</strong> {personaPorId.cedula}</p>
                            <p><strong>Género:</strong> {getGeneroText(personaPorId.genero)}</p>
                            <p><strong>Fecha Nacimiento:</strong> {formatFechaNacimientoDisplay(personaPorId.fechaNacimiento)}</p>
                            <p><strong>Lugar Nacimiento:</strong> {personaPorId.lugarNacimiento || "N/A"}</p>
                            <p><strong>Estado Civil:</strong> {personaPorId.estadoCivil || "N/A"}</p>
                            <p><strong>Dirección:</strong> {personaPorId.direccion || "N/A"}</p>
                            <p><strong>Teléfono:</strong> {personaPorId.telefono || "N/A"}</p>
                            <p><strong>Profesión:</strong> {personaPorId.profesion || "N/A"}</p>
                            <p><strong>Wallet:</strong> <span className="font-mono text-sm break-all">{personaPorId.wallet}</span></p>
                        </div>
                    )}
                </div>
            </div>
