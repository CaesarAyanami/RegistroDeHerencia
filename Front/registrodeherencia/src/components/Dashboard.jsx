...
export default function DashboardHerencia() {
  ...
  const consultarPropiedad = async () => {
    ...
    try {
      ...
      if (
        result.ciDueno === "" &&
        result.walletDueno === "0x0000000000000000000000000000000000000000"
      ) {
        setError(
          `Propiedad con ID ${idPropiedad} no encontrada o no registrada.`
        );
      } else {
        setPropiedad(result);
      }
      ...
      console.error(err);
      setError(
        "Error al consultar propiedad. Verifica que el ID sea correcto."
      );
    } finally {
      setLoading(false);
    }
  ...
  const consultarHerencia = async () => {
    ...
    try {
      ...
      if (result.length === 0) {
        setError(
          `No hay herencia definida para la propiedad ID ${idPropiedad}.`
        );
  ...
  const baseButtonClasses =
    "px-4 py-2 font-semibold text-white rounded-lg shadow-md hover:shadow-lg transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2";
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-2xl text-center max-w-sm w-full space-y-4">
          ...
          <p className="text-gray-600">
            Conecta tu billetera Ethereum para acceder al sistema.
          </p>
          {loading && (
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-2xl space-y-8">
        ...
        <section className="bg-gray-50 p-6 rounded-xl shadow-inner border border-gray-200 space-y-4">
          <h2 className="text-2xl font-bold text-gray-800 border-b pb-2">
            Consultar Propiedad y Herencia
          </h2>
            ...
            {herencia !== null && (
              <div className="p-6 bg-yellow-50 border border-yellow-300 rounded-lg shadow-md">
                <h4 className="text-xl font-bold text-yellow-800 mb-3">
                  Herencia Definida ({herencia.length} Herederos)
                </h4>
                {herencia.length > 0 ? (
                  <ul className="space-y-1">
                    {herencia.map((h, index) => (
                      <li
                        key={index}
                        className="text-sm border-b border-yellow-200 last:border-b-0 py-1"
                      >
                        <strong>CI:</strong> {h.ciHeredero},{" "}
                        <strong>Wallet:</strong>{" "}
                        {truncateAddress(h.walletHeredero)}, <strong>%:</strong>{" "}
                        {h.porcentaje}%
                      </li>
                    ))}
                  </ul>
                  ...
                  <p className="text-sm text-gray-700">
                    No hay herederos definidos para esta propiedad.
                  </p>
                )}
              </div>
            )}
